...
import { setIsOpenModalDetail, setLoading } from '../../../../redux/slices/billSlice.js';
...
function DetailBill({ getAllBill }) {
  ...
  const itemSelected = billsList[indexSelected];
  ...
  const handleCreateContract = async () => {
    try {
      const url = `${baseUrl}/bill/createContractUser/${itemSelected._id}`;
      const res = await customAxios.post(url);
      if (res.data.success) {
        toast.success('Tạo hợp đồng và gửi đến người dùng thành công', {
          position: "top-right"
        });
